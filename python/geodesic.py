...
class geodesic(ode):
    ...
    def __init__(self, r, j, Avv, M, N, x, v, lam = 0, dtd = None, atol = 1e-2, rtol = 1e-2, callback = None, parameterspacenorm = False,invSVD=True):
        """
        ...
        #self.integrator = 'dopri5' # This is a solver for non-stiff ODEs that uses a fifth-order Runge-Kutta method.
        #self.integrator = 'lsoda' # This is a solver for stiff and non-stiff ODEs that uses Adams and BDF methods.
        self.integrator = 'vode'

        print(f"integrator: {self.integrator}")
        ode.set_integrator(self, self.integrator, atol = atol, rtol = rtol)
        if callback is None:
        ...
        self.invSVD = invSVD 
        if self.invSVD:
            print("Singular Value Decomposition 'on'")
    ...
    def set_initial_value(self, x, v):
        ...
        self.gmunu = np.dot(self.j(self.xs[-1]).T,self.j(self.xs[-1]))
        self.eigen0 = np.linalg.eig(self.gmunu)[0]
    ...
    def step(self, dt = 1.0):
        ...
        self.gmunu = np.dot(self.j(self.xs[-1]).T,self.j(self.xs[-1]))
        self.eigen= np.linalg.eig(self.gmunu)[0]

    def integrate(self, tmax, maxsteps = 50000):
        """
